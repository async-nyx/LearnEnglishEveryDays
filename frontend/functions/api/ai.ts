...
export const onRequestPost = async ({ request }: { request: Request }) => {
  let body: { provider?: string; key?: string; prompt?: string; model?: string }
  try {
  ...
  // model do trình duyệt gửi lên (danh sách nằm ở frontend/src/lib/ai.ts); chặn ký tự lạ cho chắc
  const model = /^[a-zA-Z0-9._-]{1,60}$/.test(body.model ?? '') ? (body.model as string) : ''
  if (!key || !prompt) return fail('Thiếu khoá hoặc nội dung.')
  if (provider !== 'gemini' && provider !== 'grok') return fail('Nhà cung cấp không hỗ trợ.')

  try {
    if (provider === 'gemini') {
      const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model || 'gemini-3.6-flash'}:generateContent?key=${encodeURIComponent(key)}`, {
        method: 'POST',
    ...
    const r = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: model || 'grok-4-fast', messages: [{ role: 'user', content: prompt }], temperature: 0.2 }),
    })
